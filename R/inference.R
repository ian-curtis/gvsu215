...
infer_paired <- function(data, var1, var2, digits = 3, conf_lvl = 0.95, caption = NULL) {
  ...
  check_test(mosaic::t_test(x = var1, y = var2, data = data, conf.level = conf_lvl, paired = TRUE))
  ...
  diff_t <- mosaic::t_test(x = var1, y = var2, data = data, conf.level = conf_lvl, paired = TRUE)
...
infer_mean2_int <- function(data, formula, digits = 3, conf_lvl = 0.95, caption = NULL) {
  ...
  if (base::levels(base::as.factor(formula[[3]])) != 2) {
