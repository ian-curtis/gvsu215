...
```{verbatim, lang = "r"}
plot_hist(_DATANAME_, ~_VARIABLE_, breaks = seq(_START_, _STOP_, _JUMP_))
```
...
```{verbatim, lang = "r"}
plot_hist(_DATANAME_, ~_VARIABLE_, group = ~_GROUPVARIABLE_, breaks = seq(_START_, _STOP_, _JUMP_))
```
...
```{verbatim, lang = "r"}
infer_1prop_test(_DATANAME_, ~_VARIABLE_, success = "_SUCCESSCATEGORY_", conf_lvl = _CONFIDENCELEVEL_)
```

## Hypothesis Test on &mu;

```{verbatim, lang = "r"}
infer_1mean_test(_DATANAME_, ~_VARIABLE_, conf_lvl = _CONFIDENCELEVEL_)
```
